...
/**
 * Loader class for application properties. Will try to find properties in
 * nxmc.properties file, JVM options, and process environment
 * Properties passed as JVM options should have names prefixed with "nxmc.".
 * Properties passed as environment variables should have names prefixed with "NXMC_" and converted to uppercase.
 */
      ...
      if (name.equals("server"))
      {
         if (!resolveServerNameCompleted)
         {
            try
            {
               ...
               Activator.logInfo("Server address resolved from NETXMS_SERVER: " + resolvedServerAddress);
            }
   ...
   public void setProperty(String name, String value)
   {
	...
	private Properties loadPropertyFile()
	{
		...
		try
		{
			...
			{
				properties.load(in);
				Activator.logInfo("Properties loaded from nxmc.properties");
				properties.forEach(new BiConsumer<Object, Object>() {
               @Override
               public void accept(Object t, Object u)
               {
                  Activator.logInfo("   " + t + " = " + u);
               }
            });
			}
		}
