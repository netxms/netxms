...
import org.netxms.webui.design.builder.CoolbarLayerBuilder;
...
public class CoolBarManager extends CoolBarManager2
{
	...
	public Control createControl2(final Composite parent)
	{
		...
		toolbar.getParent().getParent().addControlListener(new ControlAdapter()
		{
			...
			{
				// close the overflow and update the ToolBar if the browser has
				// resized
				closeOverflow();
				update(true);
			}
	...
	public void update(final boolean force)
	{
		if ((isDirty() || force) && getControl2() != null)
		{
			...
			/*
			 * Make a list of ToolItem widgets in the tool bar for which there is
			 * no current visible contribution item. These are the widgets to be
	...
	{
		int result = 0;
		FormData spacing = dummyBuilder.getPosition(ILayoutSetConstants.COOLBAR_SPACING);
		if (spacing != null)
		{
	...
	private FormData getOverflowButtonLayoutData()
	{
		...
		FormData fdOverFlowButton = dummyBuilder.getPosition(ILayoutSetConstants.COOLBAR_BUTTON_POS);
		if (image != null)
		{
	...
	{
		ElementBuilder layerBuilder = new CoolbarLayerBuilder(overflowParent.getParent(), ILayoutSetConstants.SET_ID_OVERFLOW);
	...
	private void fillOverflowToolbar()
	{
		if (overflowToolbar == null)
		{
			...
			DummyBuilder builder = new DummyBuilder(null, ILayoutSetConstants.SET_ID_OVERFLOW);
