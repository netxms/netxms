...
import org.netxms.webui.design.builder.DummyBuilder;
import org.netxms.webui.design.managers.ItemData;
...
public class ConfigurationDialog extends PopupDialog
{
	...
	public ConfigurationDialog(final Shell parent, final int shellStyle, final IStackPresentationSite site, final ConfigAction action)
	{
		...
		this.site = site;
		this.action = action;
		builder = new DummyBuilder(parent, ILayoutSetConstants.SET_ID_CONFIG_DIALOG);
	...
	protected void adjustBounds()
	{
		...
		getShell().setBounds(bounds.x + (bounds.width / 2) - (newWidth / 2), bounds.y + (bounds.height / 2) - (newHeight / 2),
				newWidth, newHeight);
	}
	...
	private void hookViewMenuArea(final Color white, final Composite configComposite)
	{
	...
	public int open()
	{
		...
		shell.setText(Messages.get().ConfigurationDialog_ConfigurationFor + site.getSelectedPart().getName());
