...
{
   private final I18n i18n = LocalizationHelper.getI18n(AlarmStateConditionEditor.class);
   ...
   protected void createContent(ColumnFilter initialFilter)
	{
      ...
      String[] alarmStateTexts = LogLabelProvider.getAlarmStateTexts(null);
		for(int i = Alarm.STATE_OUTSTANDING; i <= Alarm.STATE_TERMINATED; i++)
         state.add(alarmStateTexts[i]);
		state.select(0);
