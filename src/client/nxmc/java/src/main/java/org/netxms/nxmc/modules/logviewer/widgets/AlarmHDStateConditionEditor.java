...
{
   private final I18n i18n = LocalizationHelper.getI18n(AlarmHDStateConditionEditor.class);
   private final String[] OPERATIONS = { i18n.tr("IS"), i18n.tr("IS NOT") };
	 ...
	 * @param parent
	 */
   public AlarmHDStateConditionEditor(Composite parent)
	{
      super(parent);
	}
   ...
   protected void createContent(ColumnFilter initialFilter)
	{
      ...
      String[] alarmHDStateTexts = LogLabelProvider.getAlarmHDStateTexts(null);
		for(int i = Alarm.HELPDESK_STATE_IGNORED; i <= Alarm.HELPDESK_STATE_CLOSED; i++)
         state.add(alarmHDStateTexts[i]);
		state.select(0);
