...
{
   private static final Logger logger = LoggerFactory.getLogger(GenericBirtChart.class);
	...
	public GenericBirtChart(Composite parent, int style)
	{
		...
		{
         logger.error("BIRT chart creation failed", e);
		}
    ...
    */
	...
	public final void paintControl(PaintEvent event)
	{
		...
		if (fullRepaint)
		{
			...
			{
            logger.error("Exception during chart generation", e);
			}
    ...
    */
	...
	{
      RGB rgb = preferenceStore.getAsColor(name);
	...
	private void paintErrorIndicator(GC gc)
	{
		...
		if (errorImage == null)
         errorImage = ResourceManager.getImageDescriptor("icons/chart_error.png").createImage();
