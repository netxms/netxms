...
/**
 * List of persistent authentication tokens issued for given user with actions for issuing new and revoking existing tokens.
 */
public class AuthenticationTokenList extends Composite
{
   private final I18n i18n = LocalizationHelper.getI18n(AuthenticationTokenList.class);
   ...
   /**
    * Create authentication token list.
    *
    * @param parent parent composite
    * @param style widget style
    * @param userId ID of the user whose tokens are managed
    * @param messageArea message area for displaying errors from background jobs
    */
   public AuthenticationTokenList(Composite parent, int style, int userId, MessageAreaHolder messageArea)
   {
      super(parent, style);
      ...
      viewer = new SortableTableViewer(this, columnNames, columnWidths, COLUMN_ID, SWT.UP, SWT.MULTI | SWT.FULL_SELECTION | SWT.BORDER);
   ...
   {
      new Job(i18n.tr("Loading authentication tokens"), null, messageArea) {
         @Override
         protected void run(IProgressMonitor monitor) throws Exception
         {
            final List<AuthenticationToken> tokens = session.getAuthenticationTokens(userId)
                  .stream()
   ...
   {
      IssueTokenDialog dlg = new IssueTokenDialog(getShell(), userId);
      dlg.open();
   ...
   private void revokeTokens()
   {
      ...
      new Job(i18n.tr("Revoking authentication tokens"), null, messageArea) {
         @Override
         protected void run(IProgressMonitor monitor) throws Exception
         {
