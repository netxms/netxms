   ...
   private void fillMetricList()
	{
      ...
      list.add(new AgentParameter("Status", "Status", DataType.INT32));
		...
		if ((object instanceof Template) || ((object instanceof AbstractNode) && ((AbstractNode)object).isManagementServer()))
		{
         ...
         list.add(new AgentParameter("Server.Certificate.ExpirationInt", "Days until server certificate expiration date", DataType.UINT64));
         list.add(new AgentParameter("Server.Certificate.ExpirationTime", "Server certificate expiration time", DataType.UINT32)); 
