...
import org.netxms.client.datacollection.InterfacePerformanceData;
 ...
 * DCI last values
 */
public class TrafficLineChart extends OverviewPageElement 
{
   private static Logger logger = LoggerFactory.getLogger(TrafficLineChart.class);

   private final I18n i18n = LocalizationHelper.getI18n(TrafficLineChart.class);

   private Chart chart;
   private InterfacePerformanceData items = null;
    ...
    * @param objectTab
    */
   public TrafficLineChart(Composite parent, OverviewPageElement anchor, ObjectView objectView, boolean utilization)
   {
   ...
   {      
      ...
      labelControl = new Composite(content, SWT.NO_BACKGROUND);
      ...
      Job job = new Job(i18n.tr("Read last DCI values"), getObjectView()) {
         ...
         protected void run(IProgressMonitor monitor) throws Exception
         {
            ...
            {               
               final Date from = new Date(System.currentTimeMillis() - chartConfiguration.getTimeRangeMillis());
               final Date to = new Date(System.currentTimeMillis());
               final InterfacePerformanceData perfData = session.findTrafficInformation(getObject());
