   ...
   public LineChart(Chart parent)
	{
      ...
      final IPlotArea plotArea = getPlotArea();
		if (showToolTips)
		{
         WidgetHelper.attachMouseTrackListener((Composite)plotArea.getControl(), new MouseTrackListener() {
				@Override
   ...
   private void setZoomEnabled(boolean enableZoom)
	{
      final IPlotArea plotArea = getPlotArea();
		if (enableZoom)
		{
         ...
         ((Canvas)plotArea.getControl()).addPaintListener(zoomPaintListener);
		}
		else
		{
			plotArea.removeMouseListener(zoomMouseListener);
         ((Canvas)plotArea.getControl()).removePaintListener(zoomPaintListener);
		}
	}
