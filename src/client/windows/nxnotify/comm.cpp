...
{
   SendMessage(hWnd, NXNM_SET_INFO_TEXT, 0, (LPARAM)pszText);
}
...
{
   appNotify.EventHandler(dwEvent, dwCode, pArg);
}
...
static DWORD WINAPI LoginThread(void *pArg)
{
   ...
   PostMessage(hWnd, NXNM_REQUEST_COMPLETED, 0, dwResult);
   return dwResult;
}
...
static DWORD WINAPI RequestThread(void *pArg)
{
   ...
   if (pData->hWnd != NULL)
      PostMessage(pData->hWnd, NXNM_REQUEST_COMPLETED, 0, dwResult);
   return dwResult;
}
