 ...
 * 
 */
package org.netxms.nxmc.alarmviewer.actions;
...
import org.netxms.nxmc.alarmviewer.Activator;
import org.netxms.nxmc.alarmviewer.AlarmView;
			...
			try
			{
				for(int i = 0; i < selection.length; i++)
					if (selection[i] instanceof NXCAlarm)
						NXMCSharedData.getSession().terminateAlarm(((NXCAlarm)selection[i]).getId());
