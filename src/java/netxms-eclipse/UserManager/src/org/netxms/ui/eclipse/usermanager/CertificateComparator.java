	...
	public int compare(Viewer viewer, Object e1, Object e2)
	{
		...
		switch((Integer)((SortableTableViewer) viewer).getTable().getSortColumn().getData("ID")) //$NON-NLS-1$
		{
			case CertificateView.COLUMN_ID:
				result = Long.compare(((Certificate)e1).getId(), ((Certificate)e2).getId());
				break;
			case CertificateView.COLUMN_TYPE:
				result = Integer.compare(((Certificate)e1).getType(),((Certificate)e2).getType());
				break;
			case CertificateView.COLUMN_COMMENTS:
				result = ((Certificate)e1).getComments().compareTo(((Certificate)e2).getComments());
				break;
			case CertificateView.COLUMN_SUBJECT:
				result = ((Certificate)e1).getSubject().compareTo(((Certificate)e2).getSubject());
				break;
			default:
				result = 0;
				break;
