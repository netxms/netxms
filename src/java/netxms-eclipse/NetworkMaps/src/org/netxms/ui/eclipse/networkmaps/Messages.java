package org.netxms.ui.eclipse.networkmaps;

import org.eclipse.osgi.util.NLS;

public class Messages extends NLS
{
	private static final String BUNDLE_NAME = "org.netxms.ui.eclipse.networkmaps.messages"; //$NON-NLS-1$
	public static String AbstractNetworkMapView_AlignToGrid;
   public static String AbstractNetworkMapView_DisplayObjectAs;
   public static String AbstractNetworkMapView_EnableAutoLayout;
   public static String AbstractNetworkMapView_Error;
   public static String AbstractNetworkMapView_Icons;
   public static String AbstractNetworkMapView_LargeLabels;
   public static String AbstractNetworkMapView_LayoutHorzTree;
   public static String AbstractNetworkMapView_Layout;
   public static String AbstractNetworkMapView_LayoutRadial;
   public static String AbstractNetworkMapView_LayoutSparseVertTree;
   public static String AbstractNetworkMapView_LayoutSpring;
   public static String AbstractNetworkMapView_LayoutVertTree;
   public static String AbstractNetworkMapView_OpenObjDetailsError;
   public static String AbstractNetworkMapView_OpenSubmap;
   public static String AbstractNetworkMapView_OpenSubmapError;
   public static String AbstractNetworkMapView_RootObjectNotFound;
   public static String AbstractNetworkMapView_RouterDirect;
   public static String AbstractNetworkMapView_RouterManhattan;
   public static String AbstractNetworkMapView_Routing;
   public static String AbstractNetworkMapView_SaveLayout;
   public static String AbstractNetworkMapView_ShowGrid;
   public static String AbstractNetworkMapView_ShowObjDetails;
   public static String AbstractNetworkMapView_ShowStatusBkgnd;
   public static String AbstractNetworkMapView_ShowStatusFrame;
   public static String AbstractNetworkMapView_ShowStatusIcon;
   public static String AbstractNetworkMapView_SmallLabels;
   public static String AbstractNetworkMapView_SnapToGrid;
   public static String AbstractNetworkMapView_Zoom;
   public static String AbstractNetworkMapView_ZoomIn;
   public static String AbstractNetworkMapView_ZoomOut;
   public static String AddGroupBoxDialog_Color;
	public static String AddGroupBoxDialog_DialogTitle;
	public static String AddGroupBoxDialog_Height;
	public static String AddGroupBoxDialog_Title;
	public static String AddGroupBoxDialog_Width;
	public static String CreateMapGroup_DialogTitle;
	public static String CreateMapGroup_JobError;
	public static String CreateMapGroup_JobName;
	public static String CreateNetworkMap_JobError;
	public static String CreateNetworkMap_JobName;
	public static String CreateNetworkMapDialog_Custom;
	public static String CreateNetworkMapDialog_IpTopology;
	public static String CreateNetworkMapDialog_L2Topology;
	public static String CreateNetworkMapDialog_MapType;
	public static String CreateNetworkMapDialog_Name;
	public static String CreateNetworkMapDialog_PleaseEnterName;
	public static String CreateNetworkMapDialog_PleaseSelectSeed;
	public static String CreateNetworkMapDialog_SeedNode;
	public static String CreateNetworkMapDialog_Title;
	public static String CreateNetworkMapDialog_Warning;
	public static String ExtendedGraphViewer_DownloadTilesError;
	public static String ExtendedGraphViewer_DownloadTilesJob;
	public static String GeneralMapPreferences_ShowBkgnd;
	public static String GeneralMapPreferences_ShowFrame;
	public static String GeneralMapPreferences_ShowIcon;
	public static String IPNeighbors_PartName;
   public static String IPRouteMap_JobError;
   public static String IPRouteMap_JobTitle;
   public static String IPRouteMap_PartTitle;
   public static String IPRouteMap_TargetObjectNotExist;
   public static String Layer2Topology_JobError;
   public static String Layer2Topology_JobTitle;
   public static String Layer2Topology_PartTitle;
	public static String MapBackground_BkgndColor;
   public static String MapBackground_BkgndImage;
   public static String MapBackground_BkgndType;
   public static String MapBackground_Error;
   public static String MapBackground_GeoLocFormatError;
   public static String MapBackground_GeoMap;
   public static String MapBackground_GroupGeoMap;
   public static String MapBackground_Image;
   public static String MapBackground_JobError;
   public static String MapBackground_JobTitle;
   public static String MapBackground_Lat;
   public static String MapBackground_Lon;
   public static String MapBackground_None;
   public static String MapBackground_ZoomLevel;
   public static String MapLinkGeneral_BasedOnObjStatus;
   public static String MapLinkGeneral_BendPoints;
   public static String MapLinkGeneral_Color;
   public static String MapLinkGeneral_CustomColor;
   public static String MapLinkGeneral_DefColor;
   public static String MapLinkGeneral_Direct;
   public static String MapLinkGeneral_Manhattan;
   public static String MapLinkGeneral_MapDefault;
   public static String MapLinkGeneral_Name;
   public static String MapLinkGeneral_NameConn1;
   public static String MapLinkGeneral_NameConn2;
   public static String MapLinkGeneral_RoutingAlg;
   public static String MapLinkGeneral_StatusObject;
   public static String MapObjectFilter_FilteringScript;
   public static String MapObjectFilter_FilterObjects;
   public static String MapObjectFilter_JobError;
   public static String MapObjectFilter_JobTitle;
   public static String MapOptions_AdvOptions;
   public static String MapOptions_CalcStatusFromObjects;
   public static String MapOptions_CustColor;
   public static String MapOptions_CustomDiscoRadius;
   public static String MapOptions_DefaultConnOptions;
   public static String MapOptions_DefaultDispOptions;
   public static String MapOptions_DefColor;
   public static String MapOptions_Direct;
   public static String MapOptions_IncludeEndNodes;
   public static String MapOptions_JobError;
   public static String MapOptions_JobTitle;
   public static String MapOptions_Manhattan;
   public static String MapOptions_RoutingAlg;
   public static String MapOptions_ShowStatusBkgnd;
   public static String MapOptions_ShowStatusFrame;
   public static String MapOptions_ShowStatusIcon;
   public static String MapOptions_TopoDiscoRadius;
   public static String MapOptions_TopologyOptions;
   public static String NetworkMapOpenHandler_Error;
	public static String NetworkMapOpenHandler_ErrorText;
	public static String ObjectTooltip_Channel;
   public static String ObjectTooltip_ChartTitle;
   public static String ObjectTooltip_mW;
   public static String ObjectTooltip_Radio;
   public static String ObjectTooltip_TXPower;
   public static String OpenMapObject_Error;
	public static String OpenMapObject_ErrorText;
	public static String PredefinedMap_AddDecoration;
   public static String PredefinedMap_AddObject;
   public static String PredefinedMap_ConfirmRemoval;
   public static String PredefinedMap_GroupBox;
   public static String PredefinedMap_Image;
   public static String PredefinedMap_LinkObjects;
   public static String PredefinedMap_MapProperties;
   public static String PredefinedMap_Properties;
   public static String PredefinedMap_RemovalConfirmationPlural;
   public static String PredefinedMap_RemovalConfirmationSingular;
   public static String PredefinedMap_RemoveFromMap;
   public static String PredefinedMap_SaveJobError;
   public static String PredefinedMap_SaveJobTitle;
   public static String ServiceComponents_Error;
	public static String ServiceComponents_PartName;
	public static String ServiceDependency_Error;
   public static String ServiceDependency_PartTitle;
   public static String ShowIPNeighbors_Error;
	public static String ShowIPNeighbors_ErrorText;
	public static String ShowIPRoute_Error;
	public static String ShowIPRoute_ErrorOpenView;
	public static String ShowIPRoute_InvalidTarget;
	public static String ShowLayer2Topology_Error;
	public static String ShowLayer2Topology_ErrorText;
	public static String ShowServiceComponents_Error;
	public static String ShowServiceComponents_ErrorText;
	public static String ShowServiceDependency_Error;
	public static String ShowServiceDependency_ErrorText;
   public static String VlanMap_IncorrectSecondaryId;
	public static String VlanMap_JobError;
   public static String VlanMap_JobTitle;
   public static String VlanMap_PartName;
   static
	{
		// initialize resource bundle
		NLS.initializeMessages(BUNDLE_NAME, Messages.class);
	}

	private Messages()
	{
 }


	private static Messages instance = new Messages();

	public static Messages get()
	{
		return instance;
	}

}
