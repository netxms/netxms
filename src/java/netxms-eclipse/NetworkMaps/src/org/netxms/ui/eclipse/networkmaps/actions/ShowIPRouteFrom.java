...
public class ShowIPRouteFrom implements IObjectActionDelegate
{
	...
	public void run(IAction action)
	{
	   if (node == null)
		...
		try
		{
			window.getActivePage().showView(IPRouteMap.ID, Long.toString(selection[0].getObjectId()) + "&" + Long.toString(node.getObjectId()), IWorkbenchPage.VIEW_ACTIVATE); //$NON-NLS-1$
		}
