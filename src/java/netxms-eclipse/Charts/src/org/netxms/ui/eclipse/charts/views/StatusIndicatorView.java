      ...
      for (int i = 1; i < fields.length; i++) {
         ...
         if (subfields.length == 6) {
            try {
               ...
               final int sourceId = Integer.parseInt(subfields[2], 10);
               final int dataType = Integer.parseInt(subfields[3], 10);
               final String name = URLDecoder.decode(subfields[4], "UTF-8");
         ...
         protected IStatus run(IProgressMonitor monitor) {
            ...
            try {
               ...
               status = new Status(Status.ERROR, Activator.PLUGIN_ID,
                     (e instanceof NXCException) ? ((NXCException) e).getErrorCode() : 0, "Cannot get DCI values: "
                           + e.getMessage(), e);
               updateInProgress = false;
            }
