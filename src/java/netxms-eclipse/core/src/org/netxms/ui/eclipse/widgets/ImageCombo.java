...
/**
 * Instances of this class are controls that allow the user to choose an item from a list of items, or optionally enter a new value
 * by typing it into an editable text field. Often, <code>Combo</code>s are used in the same place where a single selection
 * <code>List</code> widget could be used but space is limited. A <code>Combo</code> takes less space than a <code>List</code>
 * widget and shows similar information.
 * <p>
 * Note: Since <code>Combo</code>s can contain both a list and an editable text field, it is possible to confuse methods which
 * access one versus the other (compare for example, <code>clearSelection()</code> and <code>deselectAll()</code>). The API
 * documentation is careful to indicate either "the receiver's list" or the "the receiver's text field" to distinguish between the
 * two cases.
 * </p>
 * <p>
 * Note that although this class is a subclass of <code>Composite</code>, it does not make sense to add children to it, or set a
 * layout on it.
 * </p>
 * <dl>
 * <dt><b>Styles:</b></dt>
...
public class ImageCombo extends Composite
{
	 ...
	 * <p>
	 * <code>widgetSelected</code> is called when the user changes the combo's list selection. <code>widgetDefaultSelected</code> is
	 * typically called when ENTER is pressed the combo's text area.
	 * </p>
	 * 
	 * @param listener the listener which should be notified
	 * 
	...
	/**
	 * Adds the listener to the collection of listeners who will be notified when the receiver's text is verified, by sending it one
	 * of the messages defined in the <code>VerifyListener</code> interface.
	 * 
	 * @param listener the listener which should be notified
	 * 
	 * @exception IllegalArgumentException <ul>
	...
	/**
	 * Returns a <code>Point</code> whose x coordinate is the character position representing the start of the selection in the
	 * receiver's text field, and whose y coordinate is the character position representing the end of the selection. An "empty"
	...
	/**
	 * Searches the receiver's list starting at the first item (index 0) until an item is found that is equal to the argument, and
	 * returns the index of that item. If no item is found, returns -1.
	 * 
	...
	/**
	 * Searches the receiver's list starting at the first item until an item is found that is equal to the argument, and removes that
	 * item from the list.
	 * 
	 ...
	 * remains selected. Indices that are out of range are ignored.
	 * 
	 ...
	 * <p>
	 * Note: The text field in a <code>Combo</code> is typically only capable of displaying a single line of text. Thus, setting the
	 * text to a string containing line breaks or other special characters will probably cause it to display incorrectly.
	 * </p>
	 * 
