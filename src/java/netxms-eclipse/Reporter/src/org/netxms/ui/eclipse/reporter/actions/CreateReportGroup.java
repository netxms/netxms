 ...
 *
 */
public class CreateReportGroup implements IObjectActionDelegate
{
	...
	{
		final CreateObjectDialog dlg = new CreateObjectDialog(window.getShell(), "Report Group");
		if (dlg.open() != Window.OK)
			return;
		
		new ConsoleJob("Create report group", part, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor) throws Exception
			{
				NXCSession session = (NXCSession)ConsoleSharedData.getSession();
				NXCObjectCreationData cd = new NXCObjectCreationData(AbstractObject.OBJECT_REPORTGROUP, dlg.getObjectName(), parentId);
			...
			{
				return "Cannot create report group object \"" + dlg.getObjectName() + "\"";
			}
		}.start();
	}
