 ...
 */
public class NewReportExecutionForm extends Composite
{
	private IWorkbenchPart workbenchPart = null;
	private org.netxms.api.client.reporting.ReportDefinition report;
	 ...
	 * @param style
	 */
	public NewReportExecutionForm(Composite parent, int style, org.netxms.api.client.reporting.ReportDefinition report)
	{
		super(parent, style);
		this.report = report;
	...
	private void createParamEntryFields(Composite parent)
	{
		try
		{
			...
			for(org.netxms.api.client.reporting.ReportParameter p : parameters)
			{
				FieldEditor editor;
	...
	private void refreshResultList()
	{
		 ...
		 * public void run()
		 * {
		 * if (NewReportExecutionForm.this.isDisposed())
		 * return;
		 * 
		 * resultList.setInput(results.toArray());
		 * NewReportExecutionForm.this.getParent().layout(true, true);
		 * }
		 * });
		 * }
		 * 
	...
	private void deleteResults()
	{
		 ...
		 * return;
		 * 
		 * if (!MessageDialogHelper.openConfirm(getShell(), "Delete Report Results",
		 * "Do you really want to delete selected results?"))
		 * return;
		 * 
