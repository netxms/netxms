...
public class QuartzReportScheduler implements ReportScheduler {
    ...
    public QuartzReportScheduler() throws SchedulerException {
        final Properties defaultQuartzProperties = loadProperties("org/quartz/quartz.properties");
        final Properties localDefaultProperties = loadProperties("nxreporting_default.properties");
    ...
    public UUID execute(UUID jobId, int userId, UUID reportUuid, Map<String, Object> parameters) {
        ...
        Trigger trigger = newTrigger().withIdentity(reportUuid + "_trigger", reportUuid.toString()).startNow()
                .withSchedule(simpleSchedule().withIntervalInSeconds(0)).build();
    ...
    public UUID addRecurrent(UUID jobId, UUID reportUuid, int jobType, int daysOfWeek, int daysOfMonth, Date startTime,
        ...
        switch (jobType) {
            case TYPE_DAILY:
                trigger = newTrigger().withIdentity(jobId.toString() + "_trigger", reportKey)
                        .withSchedule(cronSchedule("0 " + minutes + " " + hours + " * * ?")).build();
            ...
            case TYPE_WEEKLY:
                ...
                if (sDays.length() > 0) {
                    trigger = newTrigger().withIdentity(jobId.toString() + "_trigger", reportKey)
                            .withSchedule(cronSchedule("0 " + minutes + " " + hours + " ? * " + sDays)).build();
                }
            ...
            case TYPE_MONTHLY:
                ...
                if (sDayOfM.length() > 0) {
                    trigger = newTrigger().withIdentity(jobId.toString() + "_trigger", reportKey)
                            .withSchedule(cronSchedule("0 " + minutes + " " + hours + " " + sDayOfM + " * ?")).build();
                }
                log.debug("Created monthly cron (" + "0 " + minutes + " " + hours + " " + sDayOfM + " * ?" + ")");
                break;
            case TYPE_ONCE:
                trigger = newTrigger().startAt(dateTime.getTime()).withIdentity(jobId.toString() + "_trigger", reportKey).build();
