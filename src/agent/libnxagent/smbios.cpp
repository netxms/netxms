...
bool LIBNXAGENT_EXPORTABLE SMBIOS_Parse(BYTE *(*reader)(size_t *size))
{
   ...
   if (bios == NULL)
   {
      nxlog_debug(2, _T("SMBIOS: BIOS read failed"));
      return false;
   }
   ...
   nxlog_debug(5, _T("SMBIOS: System manufacturer: %hs"), s_hardwareManufacturer);
   nxlog_debug(5, _T("SMBIOS: System product name: %hs"), s_hardwareProduct);
   nxlog_debug(5, _T("SMBIOS: BIOS vendor: %hs"), s_biosVendor);
   nxlog_debug(5, _T("SMBIOS: BIOS version: %hs"), s_biosVersion);
   nxlog_debug(5, _T("SMBIOS: BIOS address: %04X"), s_biosAddress);
