...
bool ExecuteInSession(WTS_SESSION_INFO *session, TCHAR *command, bool allSessions, DWORD *pid);
   ...
   while (!AgentSleepAndCheckForShutdown(60000))
   {
      ...
      for (DWORD i = 0; i < sessionCount; i++)
      {
         ...
         if (!found)
         {
            ...
            {
               command.append(_T("\""));
               ExecuteInSession(&sessions[i], command.getBuffer(), false, nullptr);
            }
            else
            {
...
static bool IsProcessRunning(DWORD pid)
{
   ...
   if (hProcess == nullptr)
      return false;
   bool running = (WaitForSingleObject(hProcess, 0) == WAIT_TIMEOUT);
   CloseHandle(hProcess);
   return running;
}

/**
 * Forcibly terminate process with given PID
 */
...
void ExternalSubagentWatchdog()
{
   ...
   // Instances started by this watchdog, keyed by subagent name (PID + start time)
   ...
   while (!AgentSleepAndCheckForShutdown(60000))
   {
      ...
      for(int i = 0; i < disconnectedSubagents.size(); i++)
      {
         ...
         if ((instance != nullptr) && IsProcessRunning(instance->pid))
         {
            ...
            TerminateProcessByPID(instance->pid);
         }
      ...
      for (DWORD i = 0; i < sessionCount; i++)
      {
         if ((sessions[i].State == WTSActive) || (sessions[i].State == WTSConnected))
         {
            for (int j = 0; j < subagentsToStart.size(); j++)
            {
               ...
               command.append(name);
               DWORD pid = 0;
               if (ExecuteInSession(&sessions[i], command.getBuffer(), false, &pid))
               {
