...
static bool GetDiskSize(const TCHAR *number, UINT64 *size)
{
   ...
   {
      TCHAR buffer[1024];
      nxlog_debug(7, _T("GetDiskSize: cannot open device %s (%s)"), path, GetSystemErrorText(GetLastError(), buffer, 1024));
      return false;
   }
   ...
   {
      TCHAR buffer[1024];
      nxlog_debug(7, _T("GetDiskSize: DeviceIoControl on device %s failed (%s)"), path, GetSystemErrorText(GetLastError(), buffer, 1024));
      success = false;
   }
   CloseHandle(device);
   return success;
...
static BOOL GetDiskInfo(const TCHAR *number, STORAGE_DEVICE_DESCRIPTOR *info)
{
   ...
   {
      TCHAR buffer[1024];
      nxlog_debug(7, _T("GetDiskInfo: cannot open device %s (%s)"), path, GetSystemErrorText(GetLastError(), buffer, 1024));
      return FALSE;
   }
   ...
   {
      TCHAR buffer[1024];
      nxlog_debug(7, _T("GetDiskInfo: DeviceIoControl on device %s failed (%s)"), path, GetSystemErrorText(GetLastError(), buffer, 1024));
   }
   CloseHandle(device);
   return success;
}
