 ...
 * Session pool
 */
static ObjectArray<NETCONFSession> s_sessions(16, 16, Ownership::True);
...
NETCONFSession *AcquireSession(const InetAddress& addr, uint16_t port, const TCHAR *user, const TCHAR *password, const shared_ptr<KeyPair>& keys)
{
   ...
   {
      NETCONFSession *s = s_sessions.get(i);
   ...
   NETCONFSession *session = new NETCONFSession(addr, port, InterlockedIncrement(&s_sessionId));
...
/**
 * Release NETCONF session
 * @param invalidate If true, remove session from pool regardless of connection state.
 *                   Use this when an operation failed due to session issues.
 */
void ReleaseSession(NETCONFSession *session, bool invalidate)
{
   s_lock.lock();
   session->release();
...
{
   ObjectArray<NETCONFSession> deleteList(16, 16, Ownership::True);
   while(!s_shutdownCondition.wait(30000))
   {
      ...
      {
         NETCONFSession *s = s_sessions.get(i);
         if (!s->isBusy() && (!s->isConnected() || (now - s->getLastAccessTime() > g_netconfSessionIdleTimeout)))
         {
...
/**
 * Initialize NETCONF session pool
 */
