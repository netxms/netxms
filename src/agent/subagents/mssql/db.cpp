 ...
 */
DatabaseInstance::DatabaseInstance(const DatabaseInfo *info) : m_dataLock(MutexType::FAST), m_sessionLock(MutexType::NORMAL), m_stopCondition(true)
{
...
/**
 * Read SQL Server product version
 */
void DatabaseInstance::readServerVersion()
{
   DB_RESULT hResult = DBSelect(m_session, _T("SELECT CONVERT(varchar(64), SERVERPROPERTY('ProductVersion'))"));
   m_dataLock.lock();
   if (hResult != nullptr)
   {
      DBGetField(hResult, 0, 0, m_version, sizeof(m_version) / sizeof(TCHAR));
      DBFreeResult(hResult);
   }
   else
   {
      _tcscpy(m_version, _T("unknown"));
   }
   m_dataLock.unlock();
}
 ...
 * Get server version
 */
void DatabaseInstance::getVersion(TCHAR *buffer, size_t size)
{
   m_dataLock.lock();
   _tcslcpy(buffer, m_version, size);
   m_dataLock.unlock();
}
...
{
   nxlog_debug_tag(DEBUG_TAG, 3, _T("Poller thread for database %s started"), m_info.id);
...
reconnect:
      ...
      m_session = DBConnect(g_mssqlDriver, m_info.server, (m_info.name[0] != 0) ? m_info.name : nullptr,
            (m_info.login[0] != 0) ? m_info.login : nullptr, m_info.password, nullptr, errorText);
      if (m_session == nullptr)
      {
      ...
      readServerVersion();
      nxlog_write_tag(NXLOG_INFO, DEBUG_TAG, _T("Connection with database %s restored (version %s, connection TTL %u)"),
            m_info.id, m_version, m_info.connectionTTL);
   ...
   nxlog_debug_tag(DEBUG_TAG, 3, _T("Poller thread for database %s stopped"), m_info.id);
}
...
bool DatabaseInstance::poll()
{
   ...
   for(int i = 0; g_queries[i].name != nullptr; i++)
   {
      ...
      int tagBaseLen = static_cast<int>(_tcslen(tag));
      tag[tagBaseLen++] = _T('/');
