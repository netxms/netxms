...
/**
 * Shared byte-level placeholder scan. Operates on the raw UTF-8 template bytes
 * so the result needs no re-encoding before being fed to libcurl. Placeholder
 * names are ASCII, so scanning for "${...}" is safe inside an arbitrary UTF-8
 * stream. Each recognized token is replaced with the matching pre-escaped
 * replacement (caller escapes for its target context); a null replacement or
 * an unknown token is left literal.
 *
