...
- **Fix unreleased upgrades in place.** When a bug is found in an upgrade procedure whose schema version has not shipped in a production release, edit the original `H_UpgradeFromVxx` (and its helpers) directly. Do not add a new upgrade step to convert data produced by the buggy version — only dev/tester databases ran it, and a conversion step is permanent chain surface area added just for them.
- **Empty-string filters on TEXT/CLOB columns must branch on `g_dbSyntax`.** Oracle maps `SQL_TEXT` to CLOB, which does not support `<>` comparison (`ORA-00932`), and stores empty strings as NULL — so use `WHERE col IS NOT NULL` for `DB_SYNTAX_ORACLE` and `WHERE col IS NOT NULL AND col<>''` for everything else (on other databases empty string is a real value, so the `<>''` filter must stay).
