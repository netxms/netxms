...
static iobuf_t g_iobuf[2], *g_buflist = nullptr;
static ipfix_collector_t *g_collectors = nullptr;
static ipfix_node_t *g_ipfixlist = nullptr;
static uint16_t g_lasttid; /* change this! */
static ipfix_datarecord_t g_data = { nullptr, nullptr, 0 }; /* ipfix_export */
...
int _ipfix_write_template(ipfix_t *ifh, ipfix_collector_t *col, ipfix_template_t *templ);
int _ipfix_send_message(ipfix_t *ifh, ipfix_collector_t *col, int flag, ipfix_message_t *message);
         ...
         if (nwritten <= 0)
         {
            ...
            switch (err)
            {
               case SSL_ERROR_SSL:
                  ...
                  break;
               default:
                  nxlog_debug_tag(LIBIPFIX_DEBUG_TAG, 3, _T("SSL_write failed: %d"), err);
                  _ipfix_disconnect(col);
...
static int _connect_nonb(SOCKET sockfd, struct sockaddr *saptr, socklen_t salen, int sec)
{
   int n, error;
   fd_set rset, wset;
   struct timeval tval;
   ...
   {
      nxlog_debug_tag(LIBIPFIX_DEBUG_TAG, 2, _T("[_connect_nonb] select error: sockfd not set"));
   }
#endif
...
int ipfix_snprint_int(char *str, size_t size, void *data, size_t len)
{
   ...
   switch (len)
   {
      ...
      case 8:
         ...
         return snprintf(str, size, INT64_FMTA, tmp64);
      default:
         break;
   }
...
int ipfix_snprint_uint(char *str, size_t size, void *data, size_t len)
{
   ...
   switch (len)
   {
      ...
      case 4:
         memcpy(&tmp32, data, len);
         //tmp32 = htonl( tmp32 );
         return snprintf(str, size, "%u", tmp32);
      case 8:
         memcpy(&tmp64, data, len);
         //tmp64 = HTONLL( tmp64 );
         return snprintf(str, size, UINT64_FMTA, tmp64);
      default:
         break;
   }
   return snprintf(str, size, "err");
...
int ipfix_snprint_ipaddr(char *str, size_t size, void *data, size_t len)
{
   ...
   {
      case 4:
         return snprintf(str, size, "%u.%u.%u.%u", in[0], in[1], in[2], in[3]);
      case 16:
      {
         /** change this!
          */
...
ipfix_field_t *ipfix_create_unknown_ftinfo(int eno, int type)
{
   ...
   if ((f = MemAllocStruct<ipfix_field_t>()) == NULL)
   {
      return NULL;
   }
   if ((ft = MemAllocStruct<ipfix_field_type_t>()) == NULL)
   {
      MemFree(f);
      return NULL;
   }
...
int ipfix_add_vendor_information_elements(ipfix_field_type_t *fields)
{
   ...
   while (ft->ftype != 0)
   {
      /** create new node
       */
      if ((n = MemAllocStruct<ipfix_field_t>()) == NULL)
         goto err;
      n->ft = ft;
...
//          errno = ENOTSUP;
   ...
   {
      nxlog_debug_tag(LIBIPFIX_DEBUG_TAG, 3, _T("Cannot get address of host \"%hs\": %hs\n"), server, hstrerror(h_errno));
      errno = EINVAL;
      return -1;
   }
   ...
   {
      TCHAR buffer[1024];
      nxlog_debug_tag(LIBIPFIX_DEBUG_TAG, 3, _T("socket() failed: %s"), GetLastSocketErrorText(buffer, 1024));
      return -1;
   }
#ifdef INCR_RXTX_BUFSIZE
    _adapt_sndbuf( sock );
   ...
   else if (col->protocol == IPFIX_PROTO_UDP)
   {
      /** remember address
       */
      if ((col->to = static_cast<struct sockaddr*>(MemCopyBlock(static_cast<void*>(&serv_addr), sizeof(serv_addr)))) == nullptr)
      {
         closesocket(sock);
         return -1;
      }
   ...
   {
      TCHAR buffer[1024];
      nxlog_debug_tag(LIBIPFIX_DEBUG_TAG, 5, _T("Cannot connect to %hs: %s"), server, GetLastSocketErrorText(buffer, 1024));
      return (-1);
   }
#ifdef SSLSUPPORT
   else if (col->ssl_flag)
   {
      ...
      if (col->ctx == nullptr)
      {
         ...
         const SSL_METHOD *method = (col->protocol == IPFIX_PROTO_UDP) ? DTLS_client_method() : TLS_method();
#else
         const SSL_METHOD *method = (col->protocol == IPFIX_PROTO_UDP) ? DTLSv1_client_method() : SSLv23_method();
#endif
         if (ipfix_ssl_setup_client_ctx(&col->ctx, method, col->ssl_opts) < 0)
         {
            goto err;
         }
      }
      ...
      switch (col->protocol)
      {
         case IPFIX_PROTO_TCP:
            ...
            {
               TCHAR buffer[1024];
               nxlog_debug_tag(LIBIPFIX_DEBUG_TAG, 3, _T("SSL_connect failed: %s"), GetLastSocketErrorText(buffer, 1024));
               goto err;
            }
            if ((errorCode = ipfix_ssl_post_connection_check(col->ssl, server)) != X509_V_OK)
            {
               nxlog_debug_tag(LIBIPFIX_DEBUG_TAG, 3, _T("Error: peer certificate: %hs"), X509_verify_cert_error_string(errorCode));
               nxlog_debug_tag(LIBIPFIX_DEBUG_TAG, 3, _T("Error checking SSL peer"));
            }
            nxlog_debug_tag(LIBIPFIX_DEBUG_TAG, 5, _T("SSL connection opened"));
            break;

         case IPFIX_PROTO_UDP:
         case IPFIX_PROTO_SCTP:
         default:
            nxlog_debug_tag(LIBIPFIX_DEBUG_TAG, 3, _T("Internal error: protocol not supported"));
            closesocket(sock);
            goto err;
      }
   }
    ...
    */
   {
      ...
      {
         for (cnode = (ipfix_collector_t*)node->ifh->collectors; cnode != NULL; cnode = cnode->next)
         {
            if (col == cnode)
            {
               for (tnode = node->ifh->templates; tnode != NULL; tnode = tnode->next)
               {
                  switch (col->protocol)
                  {
                     case IPFIX_PROTO_SCTP:
                     case IPFIX_PROTO_TCP:
                        if (_ipfix_write_template(node->ifh, col, tnode) < 0)
                           return -1;
                        ...
                        if (_ipfix_send_message(node->ifh, col, (col->protocol == IPFIX_PROTO_SCTP) ? 1 : 0, &col->message) < 0)
                           return -1;
...
void
_ipfix_disconnect(ipfix_collector_t *col)
{
#ifdef SSLSUPPORT
   if (col->ssl_flag)
   {
   ...
   if (((col->protocol == IPFIX_PROTO_UDP) && col->to) || ((col->protocol == IPFIX_PROTO_SCTP) && col->to))
   {
      free(col->to);
      col->to = NULL;
   }
...
int
_ipfix_send_message(ipfix_t *ifh, ipfix_collector_t *col, int flag, ipfix_message_t *message)
{
   iobuf_t *buf;
...
int
_ipfix_send_msg(ipfix_t *ifh, ipfix_collector_t *col, iobuf_t *buf)
{
   int i, retval = -1;
   ...
   if (col->message.offset)
   {
      ...
      {
         TCHAR buffer[1024];
         nxlog_debug_tag(LIBIPFIX_DEBUG_TAG, 5, _T("[ipfix] send tempates failed: %s"), GetLastSocketErrorText(buffer, 1024));
         return -1;
      }
   }
...
reconnect:
            nxlog_debug_tag(LIBIPFIX_DEBUG_TAG, 5, _T("Connection lost, reconnect"));
            _ipfix_disconnect(col);
            if (_ipfix_connect(col) < 0)
               return -1;
         }
 ...
 */
int
_ipfix_write_hdr(ipfix_t *ifh, iobuf_t *buf)
{
...
//          errno = ENOTSUP;
   ...
   if ((i = MemAllocStruct<ipfix_t>()) == NULL)
      return -1;

   if ((i->buffer = MemAllocArray<char>(IPFIX_DEFAULT_BUFLEN)) == NULL)
   {
      free(i);
      return -1;
   }
    ...
    */
   if ((node = MemAllocStruct<ipfix_node_t>()) == NULL)
   {
      free(i->buffer);
      free(i);
      return -1;
 ...
 * parameters:
 */
void _drop_collector(ipfix_collector_t **list, ipfix_collector_t *node)
{
   if (!list || !(*list) || !node)
      return;
 ...
 * return:      0 = ok, -1 = error
 */
static int _ipfix_add_collector(ipfix_t *ifh, char *host, int port, ipfix_proto_t prot, int ssl_flag, ipfix_ssl_opts_t *ssl_opts)
{
...
#else
      ipfix_ssl_init();
      if ((ssl_opts == NULL) || (ssl_opts->keyfile == NULL) || (ssl_opts->certfile == NULL))
      {
         errno = EINVAL;
         return -1;
      }
   ...
   {
      if ((strcmp(col->chost, host) == 0) && (col->cport == port) && (col->protocol == prot))
      {
         /* collector found */
         col->usecount++;
         ifh->collectors = col;
   ...
   if ((col = MemAllocStruct<ipfix_collector_t>()) == nullptr)
      return -1;

   if ((col->chost = MemCopyStringA(host)) == nullptr)
   {
      MemFree(col);
      return -1;
   }
 ...
 * return:      0 = ok, -1 = error
 */
int ipfix_add_collector(ipfix_t *ifh, char *host, int port, ipfix_proto_t prot)
{
 ...
 * return:      0 = ok, -1 = error
 */
int ipfix_add_collector_ssl(ipfix_t *ifh, char *host, int port, ipfix_proto_t prot, ipfix_ssl_opts_t *opts)
{
 ...
 * return:      0 = ok, -1 = error
 */
int ipfix_new_template(ipfix_t *ifh, ipfix_template_t **templ, int nfields)
{
   ipfix_template_t *t;
   ...
   if ((t->fields = MemAllocArray<ipfix_template_field_t>(nfields)) == nullptr)
   {
      MemFree(t);
      return -1;
   }
 ...
 * return:      0 = ok, -1 = error
 */
int ipfix_new_data_template(ipfix_t *ifh, ipfix_template_t **templ, int nfields)
{
 ...
 * return:      0 = ok, -1 = error
 */
int ipfix_add_field(ipfix_t *ifh, ipfix_template_t *templ, uint32_t eno, uint16_t type, uint16_t length)
{
   int i;

   if ((templ->nfields < templ->maxfields) && (type < IPFIX_EFT_VENDOR_BIT))
   {
      /** set template field
       */
      i = templ->nfields;
 ...
 * return:      0 = ok, -1 = error
 */
int ipfix_add_scope_field(ipfix_t *ifh, ipfix_template_t *templ, uint32_t eno, uint16_t type, uint16_t length)
{
   int i;
   ...
   if (templ->nfields < templ->maxfields)
   {
      ...
      if (templ->ndatafields)
      {
          ...
          */
         memmove(&(templ->fields[templ->nscopefields + 1]), &(templ->fields[templ->nscopefields]),
 ...
 * return:      0 = ok, -1 = error
 */
int ipfix_get_template(ipfix_t *ifh, ipfix_template_t **templ, int nfields, ...)
{
   ipfix_template_t *t;
   int i, error;
 ...
 */
int ipfix_get_template_array(ipfix_t *ifh, ipfix_template_t **templ, int nfields, int *types, int *lengths)
{
   ipfix_template_t *t;
   int i, error, len = 0;
   ...
   /** alloc mem
    */
   if ((t = MemAllocStruct<ipfix_template_t>()) == NULL)
      return -1;
   ...
   for (error = 0, i = 0; i < nfields; i++)
   {
      ...
      if ((t->fields[i].elem = ipfix_get_ftinfo(0, t->fields[i].elem->ft->ftype)) == NULL)
      {
         error = 1;
      }
   }
 ...
 */
int ipfix_make_template(ipfix_t *handle, ipfix_template_t **templ, export_fields_t *fields, int nfields)
{
   ipfix_template_t *t;
   if (ipfix_new_data_template(handle, &t, nfields) < 0)
   {
      nxlog_debug_tag(LIBIPFIX_DEBUG_TAG, 3, _T("ipfix_new_template() failed: %s"), _tcserror(errno));
      return -1;
   }

   for (int i = 0; i < nfields; i++)
   {
      if (ipfix_add_field(handle, t, fields[i].eno, fields[i].ienum, fields[i].length) < 0)
      {
         nxlog_debug_tag(LIBIPFIX_DEBUG_TAG, 3, _T("ipfix_add_field() failed: %s"), _tcserror(errno));
...
int ipfix_export(ipfix_t *ifh, ipfix_template_t *templ, ...)
{
   ...
   {
      if (g_data.addrs)
         MemFree(g_data.addrs);
      if (g_data.lens)
         MemFree(g_data.lens);
      if ((g_data.lens = MemAllocArray<uint16_t>(templ->nfields)) == nullptr)
      {
         g_data.maxfields = 0;
         return -1;
      }
      if ((g_data.addrs = MemAllocArray<void*>(templ->nfields)) == nullptr)
      {
         MemFree(g_data.lens);
         g_data.lens = nullptr;
         g_data.maxfields = 0;
...
int _ipfix_export_array(ipfix_t *ifh, ipfix_template_t *templ, int nfields, void **fields, uint16_t *lengths)
{
   int i, newset_f = 0;
   ...
   else
   {
      ...
      while (col)
      {
         if ((col->protocol == IPFIX_PROTO_UDP) && ((now - templ->tsend) > IPFIX_DFLT_TEMPLRESENDINT))
         {
            if (_ipfix_write_template(ifh, col, templ) < 0)
            {
               return -1;
...
int ipfix_export_array_with_odid(ipfix_t *ifh, uint32_t odid, ipfix_template_t *templ, int nfields, void **fields, uint16_t *lengths)
{
   int ret;
