...
NXSL_Value *NXSL_NodeClass::getAttr(NXSL_Object *object, const TCHAR *attr)
{
   ...
   Node *node = (Node *)object->getData();
   if (!_tcscmp(attr, _T("agentCertificateSubject")))
   {
      value = new NXSL_Value(node->getAgentCertificateSubject());
   }
   else if (!_tcscmp(attr, _T("agentId")))
   {
      TCHAR buffer[64];
      value = new NXSL_Value(node->getAgentId().toString(buffer));
   }
   else if (!_tcscmp(attr, _T("agentVersion")))
   {
...
NXSL_Value *NXSL_DciClass::getAttr(NXSL_Object *object, const TCHAR *attr)
{
   ...
   NXSL_Value *value = NULL;
   if (!_tcscmp(attr, _T("activeThresholdSeverity")))
   {
      value = new NXSL_Value(dci->getThresholdSeverity());
   }
   else if (!_tcscmp(attr, _T("comments")))
   {
   ...
   else if (!_tcscmp(attr, _T("hasActiveThreshold")))
   {
      value = new NXSL_Value(dci->hasActiveThreshold() ? 1 : 0);
   }
   else if (!_tcscmp(attr, _T("id")))
   {
