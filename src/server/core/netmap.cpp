...
{
   bool inTrafic = false;
   bool outTrafic = false;
   SharedObjectArray<DCObject> dcis = node->getDCObjectsByFilter([interfaceId, &inTrafic, &outTrafic] (DCObject *obj)
      {
         String tag = obj->getSystemTag();
         if (inTrafic && outTrafic)
            return false;
         if (!inTrafic && (obj->getType() == DCO_TYPE_ITEM) && (obj->getRelatedObject() == interfaceId) &&
         ...
         if (!outTrafic && (obj->getType() == DCO_TYPE_ITEM) && (obj->getRelatedObject() == interfaceId) &&
