...
{
   if (dcObject->getStatus() == ITEM_STATUS_DISABLED)
      return true;  // do not accept data for disabled DCI (e.g. pushed values)
...
void DataCollectionTarget::doInstanceDiscovery(uint32_t requestId)
{
   ...
   for(int i = 0; i < m_dcObjects.size(); i++)
   {
      ...
      if ((object->getInstanceDiscoveryMethod() != IDM_NONE) && (object->getInstanceDiscoveryMethod() != IDM_PUSH) && (object->getInstanceDiscoveryMethod() != IDM_OTLP) && (object->getStatus() != ITEM_STATUS_DISABLED))
      {
         object->setBusyFlag();
