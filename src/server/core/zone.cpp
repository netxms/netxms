...
uint32_t Zone::getProxyNodeId(NetObj *object, bool backup)
{
   ...
   if (proxy == nullptr)
   {
      SharedObjectArray<ZoneProxy>* proxyNodes = m_proxyNodes.getAll();
...
{
   SharedObjectArray<NetObj> *nodes = g_idxNodeById.getObjects(ProxyFilter, &source->nodeId);
...
json_t *Zone::toJson()
{
   ...
   SharedObjectArray<ZoneProxy>* proxyNodes = m_proxyNodes.getAll();
   json_object_set_new(root, "proxyNodeId", json_object_array(proxyNodes));
   delete proxyNodes;
