...
bool Chassis::saveToDatabase(DB_HANDLE hdb)
{
   ...
   if (success)
   {
      ...
      if (IsDatabaseRecordExist(hdb, _T("chassis"), _T("id"), m_id))
         hStmt = DBPrepare(hdb, _T("UPDATE chassis SET controller_id=?,rack_id=?,rack_image=?,rack_position=?,rack_height=?,rack_orientation=?,flags=? WHERE id=?"));
      else
         hStmt = DBPrepare(hdb, _T("INSERT INTO chassis (controller_id,rack_id,rack_image,rack_position,rack_height,rack_orientation,flags,id) VALUES (?,?,?,?,?,?,?,?)"));
      if (hStmt != NULL)
      {
...
bool Chassis::loadFromDatabase(DB_HANDLE hdb, UINT32 id)
{
   ...
   DB_STATEMENT hStmt = DBPrepare(hdb, _T("SELECT controller_id,rack_id,rack_image,rack_position,rack_height,rack_orientation,flags FROM chassis WHERE id=?"));
   if (hStmt == NULL)
      return false;
