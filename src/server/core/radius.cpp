...
int RadiusAuthEx(const wchar_t *login, const wchar_t *passwd, RADIUSChallengeData *challengeData)
{
   ...
   int result = DoRadiusAuth(rlogin, rpasswd, useSecondary, server, nullptr, 0, challengeData);
   nxlog_debug_tag(DEBUG_TAG, 4, _T("DoRadiusAuth returned %d for user %s"), result, login);
   if (CanRetry(result))
   {
      useSecondary = !useSecondary;
      ...
      result = DoRadiusAuth(rlogin, rpasswd, useSecondary, server, nullptr, 0, challengeData);
      nxlog_debug_tag(DEBUG_TAG, 4, _T("DoRadiusAuth returned %d for user %s"), result, login);
   }
   ...
   if (result == RADIUS_RESULT_CHALLENGE)
   {
      ...
      nxlog_write_tag(NXLOG_INFO, DEBUG_TAG,
            ...
            login, server, (challengeData != nullptr) ? challengeData->replyMessage : "");
   }
   else
   {
      nxlog_write_tag(NXLOG_INFO, DEBUG_TAG,
