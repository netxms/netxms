...
bool Node::loadFromDatabase(DB_HANDLE hdb, UINT32 dwId)
{
   ...
   m_requiredPollCount = DBGetFieldLong(hResult, 0, 15);
   ...
   m_agentId = DBGetFieldGUID(hResult, 0, 54);
   m_agentCertSubject = DBGetField(hResult, 0, 55, NULL, 0);
   if ((m_agentCertSubject != NULL) && (m_agentCertSubject[0] == 0))
      MemFreeAndNull(m_agentCertSubject);
   DBGetField(hResult, 0, 56, m_hypervisorType, MAX_HYPERVISOR_TYPE_LENGTH);
   m_hypervisorInfo = DBGetField(hResult, 0, 57, NULL, 0);
...
bool Node::saveToDatabase(DB_HANDLE hdb)
{
   ...
   if (success && (m_modified & MODIFY_NODE_PROPERTIES))
   {
      static const TCHAR *columns[] = {
         _T("primary_ip"), _T("primary_name"), _T("snmp_port"), _T("node_flags"), _T("snmp_version"), _T("community"),
         ...
         _T("fail_time_snmp"), _T("fail_time_agent"), _T("runtime_flags"), _T("rack_orientation"), _T("rack_image_rear"),
         _T("agent_id"), _T("agent_cert_subject"), _T("hypervisor_type"), _T("hypervisor_info"),
         NULL
      };
      ...
      if (hStmt != NULL)
      {
         ...
         DBBind(hStmt, 56, DB_SQLTYPE_VARCHAR, m_agentId);
         DBBind(hStmt, 57, DB_SQLTYPE_VARCHAR, m_agentCertSubject, DB_BIND_STATIC);
         DBBind(hStmt, 58, DB_SQLTYPE_VARCHAR, m_hypervisorType, DB_BIND_STATIC);
         DBBind(hStmt, 59, DB_SQLTYPE_VARCHAR, m_hypervisorInfo, DB_BIND_STATIC);
...
Interface *Node::createInterfaceObject(InterfaceInfo *info, bool manuallyCreated, bool fakeInterface, bool syntheticMask)
{
   ...
   if (!manuallyCreated)
   {
      ...
      NXSL_Value *argv = vm->createValue(new NXSL_Object(&g_nxslInterfaceClass, iface));
      if (vm->run(1, &argv))
      {
...
restart_agent_check:
   ...
   for(int i = 0; i < pollList.size(); i++)
   {
      ...
      switch(curr->getObjectClass())
      {
         ...
         case OBJECT_ACCESSPOINT:
            ...
            static_cast<AccessPoint*>(curr)->statusPoll(pSession, rqId, pQueue, this, snmp);
            break;
         default:
   ...
   if (m_ipAddress.isValidUnicast() || agentConnected)
   {
      ...
      unlockChildList();
      if (allDown && (m_flags & NF_IS_NATIVE_AGENT) && !(m_flags & NF_DISABLE_NXCP))
      {
         if (m_dwDynamicFlags & NDF_AGENT_UNREACHABLE)
         {
      ...
      nxlog_debug(6, _T("StatusPoll(%s): allDown=%s, dynFlags=0x%08X"), m_name, allDown ? _T("true") : _T("false"), m_dwDynamicFlags);
      if (allDown)
      {
         ...
         {
            if((m_dwDynamicFlags & NDF_NETWORK_PATH_PROBLEM) && !checkNetworkPath(rqId))
            {
               PostEvent(EVENT_NODE_DOWN, m_id, NULL);
               m_dwDynamicFlags &= ~NDF_NETWORK_PATH_PROBLEM;
            }
   ...
   else
   {
      ...
      for(UINT32 i = 0; i < g_dwNumModules; i++)
      {
         if (g_pModuleList[i].pfConfPollHook != NULL)
         {
            ...
            if (g_pModuleList[i].pfConfPollHook(this, pSession, rqId, poller))
...
Subnet *Node::createSubnet(InetAddress& baseAddr, bool syntheticMask)
{
   ...
   NXSL_VM *vm = CreateServerScriptVM(_T("Hook::CreateSubnet"));
   if (vm != NULL)
   {
      bool pass = true;
      vm->setGlobalVariable(_T("$node"), createNXSLObject());
      NXSL_Value *argv = new NXSL_Value(new NXSL_Object(&g_nxslSubnetClass, subnet));
      if (vm->run(1, &argv))
      {
         NXSL_Value *result = vm->getResult();
         if ((result != NULL) && result->isInteger())
...
void Node::deployAgentPolicies()
{
   ...
   for(int i = 0; i < policies->size(); i++)
   {
      ...
      {
         if (policy->isAutoUninstallEnabled() && policy->isDirectChild(m_id))
         {
