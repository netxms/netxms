 ...
 * Default constructor
 */
DashboardTemplate::DashboardTemplate() : super(), m_instanceDashboards(), m_nameTemplate()
{
}
 ...
 * Constructor for creating new dashboard object
 */
DashboardTemplate::DashboardTemplate(const TCHAR *name) : super(name), m_instanceDashboards(), m_nameTemplate()
{
}
...
bool DashboardTemplate::loadFromDatabase(DB_HANDLE hdb, uint32_t id, DB_STATEMENT *preparedStatements)
{
   ...
   TCHAR query[256];
   _sntprintf(query, 256, _T("SELECT num_columns,template_name FROM dashboard_templates WHERE id=%u"), id);
   DB_RESULT hResult = DBSelect(hdb, query);
   if (hResult == nullptr)
      return false;
   if (DBGetNumRows(hResult) > 0)
   ...
   _sntprintf(query, 256, _T("SELECT instance_object_id,instance_dashboard_id FROM dashboard_template_instances WHERE dashboard_id=%u"), id);
...
bool DashboardTemplate::saveToDatabase(DB_HANDLE hdb)
{
   ...
   if (success && (m_modified & MODIFY_OTHER))
   {
      static const wchar_t *columns[] = { L"num_columns", L"template_name", nullptr };
      ...
      if (success)
      {
         success = executeQueryOnObject(hdb, L"DELETE FROM dashboard_template_instances WHERE dashboard_id=?");
         lockProperties();
         if (success && (m_instanceDashboards.size() != 0))
         {
            DB_STATEMENT hStmt = DBPrepare(hdb, L"INSERT INTO dashboard_template_instances (dashboard_id,instance_object_id,instance_dashboard_id) VALUES (?,?,?)", m_instanceDashboards.size() > 1);
            if (hStmt != nullptr)
            {
               DBBind(hStmt, 1, DB_SQLTYPE_INTEGER, m_id);
...
bool DashboardTemplate::deleteFromDatabase(DB_HANDLE hdb)
{
   ...
   if (success)
      success = executeQueryOnObject(hdb, L"DELETE FROM dashboard_template_instances WHERE id=?");
   return success;
}
   ...
   for (int i = 0; i < objects->size(); i++)
   {
      ...
      else if (decision == AutoBindDecision_Unbind)
      {
         ...
         if (dashboardId != 0)
         {
            ...
            if (dashboard != nullptr)
            {
               ThreadPoolExecute(g_clientThreadPool, [dashboard] () { dashboard->deleteObject(); });
