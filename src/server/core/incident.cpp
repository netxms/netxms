...
bool IncidentComment::saveToDatabase() const
{
   ...
   static const TCHAR *columns[] = { _T("incident_id"), _T("creation_time"), _T("user_id"), _T("comment_text"), nullptr };
   DB_STATEMENT hStmt = DBPrepareMerge(hdb, _T("incident_comments"), _T("id"), m_id, columns);
   if (hStmt == nullptr)
   {
 ...
 */
Incident::Incident(uint32_t id, uint32_t sourceObjectId, const TCHAR *title, const TCHAR *description,
...
Incident::Incident(DB_RESULT hResult, int row)
{
   ...
   m_description = DBGetField(hResult, row, 6, nullptr, 0);
   m_sourceAlarmId = DBGetFieldULong(hResult, row, 7);
   m_sourceObjectId = DBGetFieldULong(hResult, row, 8);
   m_createdByUser = DBGetFieldULong(hResult, row, 9);
   m_resolvedByUser = DBGetFieldULong(hResult, row, 10);
   m_closedByUser = DBGetFieldULong(hResult, row, 11);
   m_resolveTime = static_cast<time_t>(DBGetFieldInt64(hResult, row, 12));
   m_closeTime = static_cast<time_t>(DBGetFieldInt64(hResult, row, 13));
}
...
{
   TCHAR query[256];
   _sntprintf(query, 256, _T("SELECT alarm_id FROM incident_alarms WHERE incident_id=%u"), m_id);
...
bool Incident::saveToDatabase() const
{
   ...
   static const TCHAR *columns[] = {
      _T("creation_time"), _T("last_change_time"), _T("state"), _T("assigned_user_id"),
      _T("title"), _T("description"), _T("source_alarm_id"), _T("source_object_id"),
      _T("created_by_user"), _T("resolved_by_user"), _T("closed_by_user"),
      _T("resolve_time"), _T("close_time"), nullptr
   };

   DB_STATEMENT hStmt = DBPrepareMerge(hdb, _T("incidents"), _T("id"), m_id, columns);
   if (hStmt == nullptr)
   {
      DBConnectionPoolReleaseConnection(hdb);
      return false;
   ...
   DBBind(hStmt, 6, DB_SQLTYPE_TEXT, m_description, DB_BIND_STATIC);
   DBBind(hStmt, 7, DB_SQLTYPE_INTEGER, m_sourceAlarmId);
   DBBind(hStmt, 8, DB_SQLTYPE_INTEGER, m_sourceObjectId);
   DBBind(hStmt, 9, DB_SQLTYPE_INTEGER, m_createdByUser);
   DBBind(hStmt, 10, DB_SQLTYPE_INTEGER, m_resolvedByUser);
   DBBind(hStmt, 11, DB_SQLTYPE_INTEGER, m_closedByUser);
   DBBind(hStmt, 12, DB_SQLTYPE_INTEGER, static_cast<uint32_t>(m_resolveTime));
   DBBind(hStmt, 13, DB_SQLTYPE_INTEGER, static_cast<uint32_t>(m_closeTime));
 ...
 */
void Incident::postStateChangeEvent(int oldState, int newState, uint32_t userId)
{
   EventBuilder(EVENT_INCIDENT_STATE_CHANGED, m_sourceObjectId)
      .param(_T("incidentId"), m_id)
      .param(_T("title"), m_title)
      .param(_T("oldState"), oldState)
      .param(_T("oldStateName"), GetIncidentStateName(oldState))
      .param(_T("newState"), newState)
      .param(_T("newStateName"), GetIncidentStateName(newState))
...
uint32_t Incident::assign(uint32_t userId, uint32_t assignedBy)
{
   ...
   TCHAR oldValue[32], newValue[32];
   _sntprintf(oldValue, 32, _T("%u"), oldUserId);
   _sntprintf(newValue, 32, _T("%u"), userId);
   logActivity(INCIDENT_ACTIVITY_ASSIGNED, assignedBy, oldValue, newValue, nullptr);
   ...
   EventBuilder(EVENT_INCIDENT_ASSIGNED, m_sourceObjectId)
      .param(_T("incidentId"), m_id)
      .param(_T("title"), m_title)
      .param(_T("assignedUserId"), userId)
      .param(_T("assignedByUserId"), assignedBy)
      .post();
...
/**
 * Resolve incident - also resolves all linked alarms
 */
uint32_t Incident::resolve(uint32_t userId)
{
   lock();
   ...
   int oldState = m_state;
   m_state = INCIDENT_STATE_RESOLVED;
   m_resolvedByUser = userId;
   m_resolveTime = time(nullptr);
   m_lastChangeTime = m_resolveTime;

   // Resolve all linked alarms
   for (int i = 0; i < m_linkedAlarms.size(); i++)
   {
      ResolveAlarmById(m_linkedAlarms.get(i), nullptr, false, false);
   }

   updateInDatabase();
   logActivity(INCIDENT_ACTIVITY_STATE_CHANGE, userId, GetIncidentStateName(oldState), GetIncidentStateName(INCIDENT_STATE_RESOLVED), nullptr);

   unlock();

   EventBuilder(EVENT_INCIDENT_RESOLVED, m_sourceObjectId)
      .param(_T("incidentId"), m_id)
      .param(_T("title"), m_title)
      .param(_T("resolvedByUser"), userId)
      .post();
 ...
 * Close incident
 */
uint32_t Incident::close(uint32_t userId)
{
   lock();
   ...
   int oldState = m_state;
   m_state = INCIDENT_STATE_CLOSED;
   m_closedByUser = userId;
   m_closeTime = time(nullptr);
   m_lastChangeTime = m_closeTime;

   updateInDatabase();
   logActivity(INCIDENT_ACTIVITY_STATE_CHANGE, userId, GetIncidentStateName(oldState), GetIncidentStateName(INCIDENT_STATE_CLOSED), nullptr);

   unlock();

   EventBuilder(EVENT_INCIDENT_CLOSED, m_sourceObjectId)
      .param(_T("incidentId"), m_id)
      .param(_T("title"), m_title)
      .param(_T("closedByUser"), userId)
      .post();

   NotifyClientsOnIncidentUpdate(CMD_INCIDENT_UPDATE, this);

   // Remove from active list (incident is closed)
   s_incidents.lock();
...
void Incident::fillMessage(NXCPMessage *msg) const
{
   ...
   msg->setField(VID_INCIDENT_DESCRIPTION, CHECK_NULL_EX(m_description));
   ...
   const TCHAR *description, uint32_t sourceAlarmId, uint32_t userId, uint32_t *incidentId)
{
   ...
   auto incident = make_shared<Incident>(id, sourceObjectId, title, description, sourceAlarmId, userId);
   ...
   EventBuilder(EVENT_INCIDENT_OPENED, sourceObjectId)
      .param(_T("incidentId"), id)
      .param(_T("title"), title)
      .param(_T("sourceAlarmId"), sourceAlarmId)
      .param(_T("createdByUser"), userId)
      .post();
...
uint32_t NXCORE_EXPORTABLE ResolveIncident(uint32_t incidentId, uint32_t userId)
{
   ...
   return incident->resolve(userId);
}
 ...
 * Persistent data format: "alarm=<id>;object=<id>;title=<text>;description=<text>"
 */
void CreateIncidentFromAlarmTask(const shared_ptr<ScheduledTaskParameters>& parameters)
{
   ...
   for (int i = 0; i < parts.size(); i++)
   {
      ...
      if (wcsncmp(part, L"alarm=", 6) == 0)
         ...
         wcslcpy(description, part + 12, 2001);
   }
   ...
   // Create the incident
   uint32_t incidentId;
   uint32_t rcc = CreateIncident(objectId, finalTitle, (description[0] != 0) ? description : nullptr, alarmId, 0, &incidentId);
