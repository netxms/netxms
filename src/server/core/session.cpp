	...
	if (g_flags & AF_SINGLE_TABLE_PERF_DATA)
	{
      switch(g_dbSyntax)
      {
         case DB_SYNTAX_MSSQL:
            _sntprintf(query, 512, _T("SELECT TOP %d %s_timestamp,%s_value FROM %s WHERE node_id=? AND item_id=?%s ORDER BY %s_timestamp DESC"),
                     (int)maxRows, tablePrefix, tablePrefix,
                     tablePrefix, condition, tablePrefix);
            break;
         case DB_SYNTAX_ORACLE:
            _sntprintf(query, 512, _T("SELECT * FROM (SELECT %s_timestamp,%s_value FROM %s WHERE node_id=? AND item_id=?%s ORDER BY %s_timestamp DESC) WHERE ROWNUM<=%d"),
         ...
         case DB_SYNTAX_TSDB:
            _sntprintf(query, 512, _T("SELECT %s_timestamp,%s_value FROM %s WHERE node_id=? AND item_id=?%s ORDER BY %s_timestamp DESC LIMIT %d"),
                     tablePrefix, tablePrefix,
                     tablePrefix, condition, tablePrefix, (int)maxRows);
            break;
         case DB_SYNTAX_DB2:
            _sntprintf(query, 512, _T("SELECT %s_timestamp,%s_value FROM %s WHERE node_id=? AND item_id=?%s ORDER BY %s_timestamp DESC FETCH FIRST %d ROWS ONLY"),
                     tablePrefix, tablePrefix,
                     tablePrefix, condition, tablePrefix, (int)maxRows);
            break;
         default:
