...
bool NetObj::linkObjects(const shared_ptr<NetObj>& parent, const shared_ptr<NetObj>& child)
{
   ...
   if (child->isDeleteInitiated() || parent->isDeleteInitiated())
   {
      ...
      parent->markAsModified(MODIFY_RELATIONS);   // relations are saved from the parent side only, so its next save drops the transient link if syncer already saved it
 ...
 */
void NetObj::setResponsibleUsers(StructArray<ResponsibleUser> *responsibleUsers, GenericClientSession *session)
{
   lockResponsibleUsersList();
...
/**
 * Get copy of object's own responsible users list (not including users inherited
 * from parent objects). Returns an empty array if no users are configured.
 */
unique_ptr<StructArray<ResponsibleUser>> NetObj::getResponsibleUsers() const
{
   lockResponsibleUsersList();
   auto responsibleUsers = (m_responsibleUsers != nullptr) ?
            make_unique<StructArray<ResponsibleUser>>(*m_responsibleUsers) : make_unique<StructArray<ResponsibleUser>>(0, 16);
   unlockResponsibleUsersList();
   return responsibleUsers;
}

/**
 * Add or update single responsible user. If user is already in the list its tag
 * is updated, otherwise a new entry is appended. Change events are generated by
 * setResponsibleUsers().
 */
void NetObj::updateResponsibleUser(uint32_t userId, const wchar_t *tag, GenericClientSession *session)
{
   unique_ptr<StructArray<ResponsibleUser>> list = getResponsibleUsers();
   bool found = false;
   for(int i = 0; i < list->size(); i++)
   {
      ResponsibleUser *r = list->get(i);
      if (r->userId == userId)
      {
         wcslcpy(r->tag, CHECK_NULL_EX(tag), MAX_RESPONSIBLE_USER_TAG_LEN);
         found = true;
         break;
      }
   }
   if (!found)
   {
      ResponsibleUser *r = list->addPlaceholder();
      r->userId = userId;
      wcslcpy(r->tag, CHECK_NULL_EX(tag), MAX_RESPONSIBLE_USER_TAG_LEN);
   }
   setResponsibleUsers(list.release(), session);
}

/**
 * Delete single responsible user. Returns true if user was present and removed,
 * false if user was not in the list (list left unchanged). Change events are
 * generated by setResponsibleUsers().
 */
bool NetObj::deleteResponsibleUser(uint32_t userId, GenericClientSession *session)
{
   unique_ptr<StructArray<ResponsibleUser>> list = getResponsibleUsers();
   bool found = false;
   for(int i = 0; i < list->size(); i++)
   ...
   // Empty list is represented internally as null
   setResponsibleUsers(list->isEmpty() ? nullptr : list.release(), session);
   return true;
}
