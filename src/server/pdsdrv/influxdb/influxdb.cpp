...
int InfluxDBStorageDriver::sockfd = 0;
const char *InfluxDBStorageDriver::HOSTNAME = "localhost";

/**
 * Driver name
 */
static TCHAR *s_driverName = (wchar_t *)L"InfluxDB";
...
void InfluxDBStorageDriver::queuePush(std::string data, unsigned int maxSize = 100) {
  ...
  if ((nbQueuedMessages >= maxSize) || flushNow) {
    ...
    if (queuedMessages.size() > 0) {
      if (sendto(sockfd, (char *)queuedMessages.c_str(), queuedMessages.size(), 0, (sockaddr *)&serv_addr, sizeof(serv_addr)) <= 0) {
...
bool InfluxDBStorageDriver::init() {
  ...
  if (sockfd < 0) {
    logMessage("Cannot open socket", 1);
    return false;
  }

  server = gethostbyname(HOSTNAME);
  if (server == NULL) {
    logMessage("No such destination host", 1);
    return false;
  }
