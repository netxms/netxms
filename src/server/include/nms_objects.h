...
public:
   ...
   bool ensureAggregateTable(DB_HANDLE hdb, bool hourly);
...
unique_ptr<ObjectArray<ObservationPointHostRecord>> NXCORE_EXPORTABLE GetObservationPointHostsForNode(uint32_t nodeId);
unique_ptr<ObjectArray<ObservationPointHostRecord>> NXCORE_EXPORTABLE GetObservationPointMatchedHosts(uint32_t pointId);
void NXCORE_EXPORTABLE GetObservationPointIdsForNode(uint32_t nodeId, IntegerArray<uint32_t> *pointIds);
bool NXCORE_EXPORTABLE IsNodeObserved(uint32_t nodeId);
uint32_t NXCORE_EXPORTABLE GetObservationPointActiveHosts(ObservationPoint *point, uint32_t userId, shared_ptr<Table> *table);
DataCollectionError NXCORE_EXPORTABLE GetObservationPointHostMetric(Node *node, const wchar_t *name, wchar_t *buffer, size_t size);
DataCollectionError NXCORE_EXPORTABLE GetObservationPointHostTable(Node *node, const wchar_t *name, shared_ptr<Table> *table);
DataCollectionError NXCORE_EXPORTABLE GetNodeObservationPointsTable(Node *node, shared_ptr<Table> *table);
...
public:
   ...
   const TCHAR *getAgentPlatformName() const { return m_agentPlatformName; }
...
protected:
   ...
   void reconcileElementChanges(const ObjectArray<NetworkMapElement>& newElements);
