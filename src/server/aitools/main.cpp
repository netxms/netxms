...
static void CreateAssistantFunctionList()
{
   ...
   RegisterAIAssistantFunction(
      ...
      "To act on a specific alarm afterwards, use the alarm-management skill with the alarm IDs returned by this tool.",
      {
         ...
         { "event_window_minutes", "time window for the 'recent events' section, in minutes. Clamped to [5, 1440]. Default: 60", "integer" },
      ...
      "@data-collection.md",
      {
         ...
         AssistantFunction(
            ...
            "Get historical time series data for analysis and anomaly detection. Returns data points with timestamps and values for specified time period.",
            {
               ...
               { "timeFrom", "start time as string: ISO 8601 (server local time unless Z or UTC offset is given), UNIX timestamp, or relative like '-60' (minutes ago), '-2h', '-7d' (default: -1440, i.e. last 24 hours)" },
               { "timeTo", "end time as string in the same formats as timeFrom (optional, defaults to now)" },
