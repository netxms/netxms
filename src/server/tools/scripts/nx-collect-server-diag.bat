...
if /I NOT "{syslog}" == %netxmslog% copy %netxmslog% %logdir%\ 2>>%errorlog% ELSE echo Server log in syslog >> %errorlog%

For /f "tokens=1* delims=^" %%a in ('%execdir%nxagentd -l') do (set netxmslog=%%a)
if /I NOT "{syslog}" == %netxmslog% copy %netxmslog% %logdir%\ 2>>%errorlog% ELSE echo Agent log in syslog >> %errorlog%

REM Zip DeleteFolder

CScript  %execdir%zip.vbs  %logdir%  %logdir%.zip
REM will not delete folder in case if zip will not succeed in time
