      ...
      if (hStmt != nullptr)
      {
         ...
         for (int i = 0; (i < count) && success; i++)
         {
            ...
            if (data == nullptr)
            {
               ...
               // migration ran (issue #3359). Backfill protected stock images from the
               // copies embedded in stock_images.cpp; for user images, warn and insert a
               // placeholder so the upgrade can complete instead of aborting on a single
               // lost file.
               const char *encoded = FindStockImageData(guid);
               if (encoded == nullptr)
               {
