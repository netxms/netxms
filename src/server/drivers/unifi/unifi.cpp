...
namespace UnifiHelpers
{
    ...
    static bool MatchStaToRadio(WirelessStationInfo *sta, const StructArray<RadioInterfaceInfo> *radios)
    {
        ...
        for (int i = 0; i < radios->size(); i++)
        {
            ...
            if ((memcmp(sta->bssid, r->bssid, MAC_ADDR_LENGTH) == 0) && (!_tcsicmp(sta->ssid, r->ssid)))
            {
                ...
                {
                    nxlog_debug_tag(DEBUG_TAG_UNIFI, 6, _T("Matched station MAC=%hs SSID=\"%s\" → radio index=%u name=\"%s\""),
    ...
    static json_t *GetStationArrayFromJson(const StringBuffer &jsonText)
    {
        ...
        if (root == nullptr)
        {
            nxlog_debug_tag(DEBUG_TAG_UNIFI, 3, _T("GetStationArrayFromJson: JSON parse error at line %d: %hs"), err.line, err.text);
            return nullptr;
        }
...
namespace UnifiSnmpHelpers
{
    ...
    static uint32_t NameWalkHandler(SNMP_Variable *v, SNMP_Transport *t, void *context)
    {
        ...
        if ((idx > 0) && (idx <= (uint32_t)ctx->radios->size()))
        {
            ...
            if (r != nullptr)
            {
                ...
                nxlog_debug_tag(DEBUG_TAG_UNIFI, 7, _T("[SNMP] NameWalkHandler: radio index=%u name=\"%s\" mapped ifIndex=%u"), r->index, r->name, r->ifIndex);
            }
        }
        return SNMP_ERR_SUCCESS;
    }
    ...
    static uint32_t SsidWalkHandler(SNMP_Variable *v, SNMP_Transport *t, void *context)
    {
        ...
        if ((idx > 0) && (idx <= (uint32_t)ctx->radios->size()))
        {
            RadioInterfaceInfo *r = ctx->radios->get(idx - 1);
            if (r != nullptr)
                nxlog_debug_tag(DEBUG_TAG_UNIFI, 7, _T("[SNMP] SsidWalkHandler: radio index=%u SSID=\"%s\""), r->index, r->ssid);
        }
        ...
        if (v->getType() == ASN_INTEGER)
        {
            ...
            if ((idx > 0) && (idx <= (uint32_t)ctx->radios->size()))
            {
                ...
                if (r != nullptr)
                {
                    ...
                    nxlog_debug_tag(DEBUG_TAG_UNIFI, 7, _T("[SNMP] ChannelWalkHandler: radio index=%u channel=%u freq=%u band=%s"), r->index, r->channel, r->frequency, RadioBandDisplayName(r->band));
                }
            }
        }
        return SNMP_ERR_SUCCESS;
        ...
        if (v->getType() == ASN_INTEGER)
        {
            ...
            if ((idx > 0) && (idx <= (uint32_t)ctx->radios->size()))
            {
                ...
                if (r != nullptr)
                {
                    ...
                    nxlog_debug_tag(DEBUG_TAG_UNIFI, 7, _T("[SNMP] TxPowerDBmWalkHandler: radio index=%u power=%d dBm"), r->index, r->powerDBm);
                }
            }
        }
        return SNMP_ERR_SUCCESS;
    ...
    static uint32_t VapModeWalkHandler(SNMP_Variable *v, SNMP_Transport *, void *context)
    {
        ...
        nxlog_debug_tag(DEBUG_TAG_UNIFI, 7, _T("[SNMP] VapModeWalkHandler: VAP %u mode=\"%s\""), vapIndex, buf);
        return SNMP_ERR_SUCCESS;
    }
    ...
    static uint32_t VapRadioIndexWalkHandler(SNMP_Variable *v, SNMP_Transport *, void *context)
    {
        ...
        nxlog_debug_tag(DEBUG_TAG_UNIFI, 7, _T("[SNMP] VapRadioIndexWalkHandler: VAP %u → radioIndex=%u"), vapIndex, radioIndex);
        return SNMP_ERR_SUCCESS;
    }
    ...
    static uint32_t RadioNameWalkHandler(SNMP_Variable *v, SNMP_Transport *, void *context)
    {
        ...
        nxlog_debug_tag(DEBUG_TAG_UNIFI, 7, _T("[SNMP] RadioNameWalkHandler: radioIndex=%u name=\"%s\" assigned to VAPs"), radioIndex, buf);
        return SNMP_ERR_SUCCESS;
    }
    ...
    static uint32_t BssidWalkHandler(SNMP_Variable *v, SNMP_Transport *, void *context)
    {
        ...
        if ((v->getType() == ASN_OCTET_STRING) && (v->getValueLength() == MAC_ADDR_LENGTH))
        {
            ...
            ctx->radios->add(r);
            nxlog_debug_tag(DEBUG_TAG_UNIFI, 7, _T("[SNMP] BssidWalkHandler: discovered radio index=%u BSSID=%s"), r.index, MacAddress(r.bssid, MAC_ADDR_LENGTH).toString().cstr());
        }
        return SNMP_ERR_SUCCESS;
    }
} // namespace UnifiSnmpHelpers
...
void NormalizeControllerURL(StringBuffer &url)
{
    ...
    nxlog_debug_tag(DEBUG_TAG_UNIFI, 5, _T("NormalizeControllerURL: [%s] -> [%s]"), before.cstr(), url.cstr());
}
...
{
    nxlog_debug_tag(DEBUG_TAG_UNIFI, 5, _T("LoadUnifiConfig: reading config"));
    ...
    nxlog_debug_tag(DEBUG_TAG_UNIFI, 5, _T("LoadUnifiConfig: url=[%s] site=[%s] token=%s verifyTLS=%d"),
...
{
    nxlog_debug_tag(DEBUG_TAG_UNIFI, 4, _T("UbiquitiUniFiDriver initialized (version %s)"), NETXMS_VERSION_STRING);
}
...
int UbiquitiUniFiDriver::isPotentialDevice(const SNMP_ObjectId &oid)
{
    ...
    nxlog_debug_tag(DEBUG_TAG_UNIFI, 5, _T("UbiquitiUniFiDriver::isPotentialDevice: oid=%s match=%d"), oid.toString().cstr(), match ? 1 : 0);
    return match ? 210 : 0;
}
...
bool UbiquitiUniFiDriver::isDeviceSupported(SNMP_Transport *snmp, const SNMP_ObjectId &)
{
    ...
    nxlog_debug_tag(DEBUG_TAG_UNIFI, 5, _T("UbiquitiUniFiDriver::isDeviceSupported: rc=%u model=\"%s\" supported=%d"), rc, (model[0] != 0) ? model : _T("<empty>"), supported ? 1 : 0);
    return supported;
}
...
StructArray<RadioInterfaceInfo> *UbiquitiUniFiDriver::getRadioInterfaces(SNMP_Transport *snmp, NObject *node, DriverData *)
{
    ...
    {
        nxlog_debug_tag(DEBUG_TAG_UNIFI, 4, _T("UbiquitiUniFiDriver::getRadioInterfaces: no controller config, returning nullptr"));
        return nullptr;
    }
    ...
    for (auto &it : ctx.vapInfoMap)
        nxlog_debug_tag(DEBUG_TAG_UNIFI, 5, _T("VAP %u → radioIndex=%u, radioName=\"%s\", mode=\"%s\""), it.first, it.second.radioIndex, it.second.radioName.c_str(), it.second.mode.c_str());
    ...
    nxlog_debug_tag(DEBUG_TAG_UNIFI, 5, _T("UbiquitiUniFiDriver::getRadioInterfaces: discovered %d radios"), (int)radios->size());
...
static bool UnifiLogin(CURL *curl, const UnifiConfig &cfg, bool isUnifiOS)
{
    ...
    nxlog_debug_tag(DEBUG_TAG_UNIFI, 4, _T("UnifiLogin: url=%s user=%s isUnifiOS=%d rc=%d code=%ld"),
...
ObjectArray<WirelessStationInfo> *UbiquitiUniFiDriver::getWirelessStations(SNMP_Transport *snmp, NObject *node, DriverData *)
{
    ...
    if (!LoadUnifiConfig(node, cfg))
    {
        nxlog_debug_tag(DEBUG_TAG_UNIFI, 4, _T("%s: no controller config"), __func__);
        return nullptr;
    }
    ...
    if (curl == nullptr)
    {
        nxlog_debug_tag(DEBUG_TAG_UNIFI, 3, _T("%s: curl init failed"), __func__);
        return nullptr;
    }
    ...
    {
        nxlog_debug_tag(DEBUG_TAG_UNIFI, 3, _T("%s: failed to fetch stations"), __func__);
    }
...
NDD_DRIVER(UbiquitiUniFiDriver)
NDD_END_DRIVER_LIST
