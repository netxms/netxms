 ...
 * @param oid Device OID
 */
bool AristaEOSDriver::isDeviceSupported(DeviceContext *context, const SNMP_ObjectId& oid)
{
   return true;
}
 ...
 */
void AristaEOSDriver::getModuleLayout(DeviceContext *context, NObject *node, DriverData *driverData, int module, NDD_MODULE_LAYOUT *layout)
{
 ...
 */
InterfaceList *AristaEOSDriver::getInterfaces(DeviceContext *context, NObject *node, DriverData *driverData, bool useIfXTable)
{
...
void AristaEOSDriver::getSSHDriverHints(SSHDriverHints *hints) const
{
   ...
   hints->errorPattern = "^%\\s*(Invalid input|Incomplete command|Ambiguous command|Error|Access denied)";
}
