...
#define Pivot(swapper,sz)			\
  if (last-first>PIVOT_THRESHOLD*sz) mid=pivot_big(first,mid,last,sz,context,compare);\
  else {	\
    if (compare(context,first,mid)<0) {			\
      if (compare(context,mid,last)>0) {		\
        swapper(mid,last);			\
        if (compare(context,first,mid)>0) swapper(first,mid);\
      }						\
    }						\
    else {					\
      if (compare(context,mid,last)>0) swapper(first,last)\
      else {					\
        swapper(first,mid);			\
        if (compare(context,mid,last)>0) swapper(mid,last);\
      }						\
    }						\
    first+=sz; last-=sz;			\
  }
...
#define Partition(swapper,sz) {			\
  do {						\
    while (compare(context,first,pivot)<0) first+=sz;	\
    while (compare(context,pivot,last)<0) last-=sz;	\
    if (first<last) {				\
      swapper(first,last);			\
      first+=sz; last-=sz; }			\
...
#define Insertion(swapper)			\
  ...
  for (first=((char*)base)+size;first!=last;first+=size) {	\
    ...
    for (test=first-size;compare(context,test,first)>0;test-=size) ;	\
    test+=size;					\
    if (test!=first) {				\
...
#endif
   m1 = compare(context, a, b)<0 ?
      (compare(context, b, c)<0 ? b : (compare(context, a, c)<0 ? c : a))
      : (compare(context, a, c)<0 ? a : (compare(context, b, c)<0 ? c : b));
   }
   { char *a = mid - d, *b = mid, *c = mid + d;
#ifdef DEBUG_QSORT
   fprintf(stderr, ". %d %d %d @ %p %p %p\n", *(int*)a, *(int*)b, *(int*)c, a, b, c);
#endif
   m2 = compare(context, a, b)<0 ?
      (compare(context, b, c)<0 ? b : (compare(context, a, c)<0 ? c : a))
      : (compare(context, a, c)<0 ? a : (compare(context, b, c)<0 ? c : b));
   }
...
#endif
   return compare(context, m1, m2)<0 ?
      (compare(context, m2, m3)<0 ? m2 : (compare(context, m1, m3)<0 ? m3 : m1))
      : (compare(context, m1, m3)<0 ? m1 : (compare(context, m2, m3)<0 ? m3 : m2));
}
...
static void qsort_words(void *base, size_t nmemb, void *context, int (*compare)(void *, const void *, const void *))
{
   ...
   for (first = ((char*)base) + WORD_BYTES; first != last; first += WORD_BYTES) {
      ...
      *(int*)pivot = *(int*)first;
      for (; compare(context, pl, pivot)>0; pr = pl, --pl) {
         *pr = *pl;
      }
...
void LIBNETXMS_EXPORTABLE QSort(void *base, size_t nmemb, size_t size, int (*compare)(void *, const void *, const void *), void *context)
{
   ...
   if ((CAST_FROM_POINTER(base, int) | size)&(WORD_BYTES - 1))
      qsort_nonaligned(base, nmemb, size, context, compare);
   else if (size != WORD_BYTES)
      qsort_aligned(base, nmemb, size, context, compare);
   else
      qsort_words(base, nmemb, context, compare);
}
