...
#define Pivot(swapper,sz)			\
  if (last-first>PIVOT_THRESHOLD*sz) mid=pivot_big(first,mid,last,sz,arg,compare);\
  else {	\
    if (compare(first,mid,arg)<0) {			\
      if (compare(mid,last,arg)>0) {		\
        swapper(mid,last);			\
        if (compare(first,mid,arg)>0) swapper(first,mid);\
      }						\
    }						\
    else {					\
      if (compare(mid,last,arg)>0) swapper(first,last)\
      else {					\
        swapper(first,mid);			\
        if (compare(mid,last,arg)>0) swapper(mid,last);\
      }						\
    }						\
    first+=sz; last-=sz;			\
  }
...
#define Partition(swapper,sz) {			\
  int swapped=0;				\
  do {						\
    while (compare(first,pivot,arg)<0) first+=sz;	\
    while (compare(pivot,last,arg)<0) last-=sz;	\
    if (first<last) {				\
...
#define PreInsertion(swapper,limit,sz)		\
  ...
  while (last!=(char *)base) {				\
    if (compare(first,last,arg)>0) first=last;	\
    last-=sz; }					\
  if (first!=(char *)base) swapper(first,(char*)base);
...
#define Insertion(swapper)			\
  ...
  for (first=((char*)base)+size;first!=last;first+=size) {	\
    ...
    for (test=first-size;compare(test,first,arg)>0;test-=size) ;	\
    test+=size;					\
    if (test!=first) {				\
...
static char * pivot_big(char *first, char *mid, char *last, size_t size, void *arg,
                        int compare(const void *, const void *, void *))
{
  int d=(int)((((last-first)/size)>>3)*size);
  char *m1,*m2,*m3;
  { char *a=first, *b=first+d, *c=first+2*d;
#ifdef DEBUG_QSORT
fprintf(stderr,"< %d %d %d\n",*(int*)a,*(int*)b,*(int*)c);
#endif
    m1 = compare(a,b,arg)<0 ?
           (compare(b,c,arg)<0 ? b : (compare(a,c,arg)<0 ? c : a))
         : (compare(a,c,arg)<0 ? a : (compare(b,c,arg)<0 ? c : b));
  }
  { char *a=mid-d, *b=mid, *c=mid+d;
#ifdef DEBUG_QSORT
fprintf(stderr,". %d %d %d\n",*(int*)a,*(int*)b,*(int*)c);
#endif
    m2 = compare(a,b,arg)<0 ?
           (compare(b,c,arg)<0 ? b : (compare(a,c,arg)<0 ? c : a))
         : (compare(a,c,arg)<0 ? a : (compare(b,c,arg)<0 ? c : b));
  }
...
#endif
  return compare(m1,m2,arg)<0 ?
           (compare(m2,m3,arg)<0 ? m2 : (compare(m1,m3,arg)<0 ? m3 : m1))
         : (compare(m1,m3,arg)<0 ? m1 : (compare(m2,m3,arg)<0 ? m3 : m2));
}

/* ---------------------------------------------------------------------- */

static void qsort_nonaligned(void *base, size_t nmemb, size_t size, void *arg,
           int (*compare)(const void *, const void *, void *)) {
...
static void qsort_aligned(void *base, size_t nmemb, size_t size, void *arg,
           int (*compare)(const void *, const void *, void *)) {
...
static void qsort_words(void *base, size_t nmemb, void *arg,
           int (*compare)(const void *, const void *, void *)) {
...
fprintf(stderr,"pivot=%d\n",*(int*)pivot);
  ...
  for (first=((char*)base)+WORD_BYTES;first!=last;first+=WORD_BYTES) {
    ...
    *(int*)pivot=*(int*)first;
    for (;compare(pl,pivot,arg)>0;pr=pl,--pl) {
      *pr=*pl; }
                                  ...
                                  int (*compare)(const void *, const void *, void *))
{
  ...
  if ((CAST_FROM_POINTER(base,int)|size)&(WORD_BYTES-1))
    ...
    qsort_words(base,nmemb,arg,compare);
}
