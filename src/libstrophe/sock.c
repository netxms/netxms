...
int sock_srv_lookup(const char *service, const char *proto, const char *domain, char *resulttarget, int resulttargetlength, int *resultport)
{
	...
	if (hdnsapi = LoadLibrary(_T("dnsapi.dll"))) {
	...
	if (!numdnsservers)
	{
		...
		if (hiphlpapi = LoadLibrary(_T("Iphlpapi.dll")))
		{
	...
	if (!numdnsservers)
	{
		...
		error = RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters"), 0, KEY_READ, &search);

		if (error != ERROR_SUCCESS)
		{
			error = RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("SYSTEM\\CurrentControlSet\\Services\\VxD\\MSTCP"), 0, KEY_READ, &search);
		}
	...
	if (!numdnsservers)
	{
		...
		error = RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters\\Interfaces"), 0, KEY_READ, &searchlist);
		...
		if (error == ERROR_SUCCESS)
		{
			...
			RegQueryInfoKeyA(searchlist, NULL, NULL, NULL, &numinterfaces, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
			...
			for (i = 0; i < numinterfaces; i++)
			{
				...
				RegEnumKeyExA(searchlist, i, (LPSTR)name, &len, NULL, NULL, NULL, NULL);
